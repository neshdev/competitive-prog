...
default = 0, [0 for i in range(MAX_ELEMENTS)]
...
def merge(a,b):
    ...
    for i in range(MAX_ELEMENTS):
       freq.append(freqa[i] + freqb[i])
    inv = 0
    for i in range(MAX_ELEMENTS):
        for j in range(i):
            # if freqa[i] and freqb[j] and j < i:
            inv += freqb[j]*freqa[i] 
    return inva + invb + inv, freq

def construct(x):
    freq = [0 for i in range(MAX_ELEMENTS)]
    freq[x] = 1
    return 0,freq
